...
public class QndThriftClientSslMultiThreads extends BaseQndThriftClient {
    public static void main(String[] args) throws Exception {
        ...
        try (ThriftApiClient client = ThriftApiUtils
                .createThriftApiClientSsl(hostsAndPorts, true, truststore.getAbsolutePath(),
                        truststorePass)) {
                ...
                threads[i] = new Thread(() -> {
                    try {
                        for (int j = 0; j < numCallsPerThreads; j++) {
                            ...
                            //                            TApiResult result1 = client.check(apiAuth);
                            //                            ApiResult apiResult1 = ThriftApiUtils.toApiResult(result1);
                            //                            int d1 = DPathUtils
